...
def condition_prior_adaptive(scale, label, dim, tightness=1.5):
	"""
	Concept-conditioned prior with uncertainty-aware variance.

	This replaces condition_prior. The prior mean shifts with the label
	value exactly as in the original CausalVAE. The variance is no longer
	fixed at one — it tightens for label values near 0 or 1 (high
	confidence) and loosens for values near 0.5 (ambiguous). This is a
	modification to the prior distribution within the existing KL
	divergence term, not a new loss term. It allows the encoder more
	freedom to place latent codes based on visual evidence when the
	physics-informed proxy label carries genuine uncertainty.

	For binary observable labels (always 0 or 1): confidence = 1.0,
	variance = 1 / (1 + 1.5) = 0.40. Tighter than unit variance,
	appropriate for certain supervision.

	For soft latent labels near 0.5: confidence near 0, variance near
	1.0. Same as a standard normal prior, giving the encoder freedom.

	Args:
		scale:     np.ndarray (N_CONCEPTS, 2)
		label:     tensor (batch, N_CONCEPTS)
		dim:       int, Z2_DIM
		tightness: float, controls how much confidence narrows variance

	Returns:
		mean: tensor (batch, N_CONCEPTS, dim)
		var:  tensor (batch, N_CONCEPTS, dim)
	"""
	mean = torch.ones(label.size(0), label.size(1), dim)
	var  = torch.ones(label.size(0), label.size(1), dim)
	...
	for i in range(label.size(0)):
		for j in range(label.size(1)):
			mul        = (float(label[i][j]) - scale[j][0]) / scale[j][1]
			confidence = abs(float(label[i][j]) - 0.5) * 2.0
			mean[i][j] = torch.ones(dim) * mul
			var[i][j]  = torch.ones(dim) * (1.0 / (1.0 + tightness * confidence))
...
def load_model_by_name(model, global_step, strict=True):
	"""
	...
	Args:
		...
		strict: bool: (): Whether to require an exact key match (set False when
	...
	if missing:
		print("Warning: missing keys (will use random init): {}".format(missing))
	if unexpected:
		print("Warning: unexpected keys (ignored): {}".format(unexpected))
